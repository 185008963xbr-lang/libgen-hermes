...
MIRRORS_DETAIL = [
    ...
    "https://libgen.rs/book/index.php",
    "https://libgen.st/book/index.php",
]
...
def search(query, fmt="", limit=10):
    """Search LibGen, return list of results."""
    ...
    for mirror in MIRRORS_SEARCH:
        ...
        try:
            ...
            # Parse simple results table
            rows = re.findall(
                r'<tr[^>]*>.*?<td[^>]*>.*?<a[^>]*href="[^"]*md5=([a-f0-9]+)"[^>]*>([^<]+)</a>.*?<td[^>]*>([^<]*)</td>.*?<td[^>]*>([^<]*)</td>.*?<td[^>]*>([^<]*)</td>.*?<td[^>]*>([^<]*)</td>.*?<td[^>]*>([^<]*)</td>',
                html, re.DOTALL
            )
            if rows:
                for r in rows:
                    md5, title, author, _, ext, size, _ = [x.strip() for x in r]
                ...
                log(f"  → {len(results)} 条结果")
                return results
            # Try alternate table format
            rows2 = re.findall(
                r'<a[^>]*href="[^"]*book/index\.php[^"]*md5=([a-f0-9]+)[^"]*"[^>]*>([^<]+)</a>',
                html
            )
            if rows2:
                seen_md5 = set()
                for md5, title in rows2:
                    md5 = md5.lower()
                    if md5 not in seen_md5:
                        seen_md5.add(md5)
                        results.append({"md5": md5, "title": title.strip(), "author": "", "extension": "", "size": ""})
                log(f"  → {len(results)} 条结果(简版)")
                return results
            log(f"  → 未解析到结果")
        except Exception as e:
            log(f"  ⚠️ {mirror}: {str(e)[:60]}")

    return results

def get_download_url(md5):
    """Get working download URL for an MD5."""
    for tmpl in DOWNLOAD_PROVIDERS:
        url = tmpl.format(md5=md5)
        try:
            data = http_get(url, timeout=15)
            if len(data) > 1024:
                # Check if it's a real file (not error HTML)
                if b"<html" not in data[:100]:
                    return url
                # Try to find direct link in HTML
...
def download(md5, output_dir):
    ...
    try:
        ...
        # Validate
        if len(data) < 50 * 1024:  # < 50KB
            log(f"  ⚠️ 文件太小 ({len(data)} bytes), 可能是错误页")
            return False
        ...
        with open(path, "wb") as f:
            f.write(data)
        log(f"  ✅ 保存: {path} ({len(data)//1024}KB)")
        return True
    except Exception as e:
        log(f"  ❌ 下载失败: {e}")
        return False
...
if __name__ == "__main__":
    ...
    if cmd == "search":
        ...
        print(json.dumps({"success": ok, "md5": md5, "output_dir": out_dir}, ensure_ascii=False))
        ...
        for line in lines:
            ...
            if re.match(r'^[a-f0-9]{32}$', parts[0].lower()):
                ...
                if results:
                    ...
                    report.append({"query": query, "status": "not_found"})
                    continue
            else:
                continue
            ok = download(md5, out_dir)
            report.append({"query": parts[0] if parts else md5, "md5": md5, "status": "success" if ok else "failed"})
        with open(os.path.join(out_dir, "download_report.json"), "w") as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        log(f"\n批量完成: {sum(1 for r in report if r['status']=='success')}/{len(report)} 成功")
